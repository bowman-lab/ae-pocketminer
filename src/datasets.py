...
# Set to True to re-enable verbose per-file loading prints.
DEBUG = False
...
DATA_DIR = os.environ.get('POCKETMINER_DATA_DIR', '../data/')

# --- DATASET VARIANTS ---
# Which training-data variant to load from data/task2/, as a filename
# suffix. Overridable via the TRAIN_DATA_SUFFIX environment variable (set
# from train_xtal_predictor.py based on the yaml `train_data_suffix` key).
#   ''                          -> original PocketMiner dataset
#                                  (task2_mod_path/X-train-{filestem}.npy)
#   '_plus_36prots_fah_rmVP35'  -> + proteins, VP35 removed
#                                  (the largest dataset used for AE-PocketMiner)
TRAIN_DATA_SUFFIX = os.environ.get('TRAIN_DATA_SUFFIX', '')


def determine_global_weights(filestem, positive_cutoff, negative_cutoff):
    y_train = np.load(os.path.join(DATA_DIR, f"task2_mod_path/y-train-{filestem}{TRAIN_DATA_SUFFIX}.npy"),
                       allow_pickle=True)
...
def simulation_dataset(batch_size, filestem, use_tensors=True, y_type='int32', use_lm=False):
    ...
    '''
    # TRAIN_DATA_SUFFIX selects which pre-built training-data variant to load
    # (see module-level comment above); '' reproduces the original behavior.
    X_train = np.load(os.path.join(DATA_DIR, f"task2_mod_path/X-train-{filestem}{TRAIN_DATA_SUFFIX}.npy"))
    y_train = np.load(os.path.join(DATA_DIR, f"task2_mod_path/y-train-{filestem}{TRAIN_DATA_SUFFIX}.npy"),
                       allow_pickle=True)
    if DEBUG:
...
def parse_batch(batch, use_tensors=True, y_type='int32', use_lm=False):
    ...
    for i, ex in enumerate(batch):
        ...
        if use_tensors:
            ...
            X_path = os.path.join(DATA_DIR, 'precompute_X', fn)
            if DEBUG:
                print(f'Loading {X_path}...', flush=True)
            xyz = np.load(X_path)
        else:
        ...
        if use_lm:
            ...
            S_path = os.path.join(DATA_DIR, 'precompute_S', fn)
