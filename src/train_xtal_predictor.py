...
#### GPU INFO ####
# To avoid err, Physical devices cannot be modified after being initialized
# config GPUs right after importing tf
...
def make_model():
    if use_pretrained:
        model = MQAModel(node_features=(8, 100), edge_features=(1,32),
                         ...
                         use_attention=use_attention, attention_heads=NUM_HEADS,
...
def main():
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        #     mask, xtal_val_ids, val_label_dicts)
    ...
    predictions, mask = predict_on_xtals(model, test_ids, 
        use_tensors=use_tensors, use_lm=use_lm)
    # predictions, mask = predict_on_xtals(model, test_ids,
    #     use_tensors=use_tensors_in_test_set, use_lm=False)
    ...
    #     mask, test_ids, test_label_dicts, test=True)
...
def assess_performance(predictions, mask, label_dicts, test=False):
    ...
    # protein_aucs = [roc_auc_score(np.array(t[l]), np.array(p[l]))
    #                 for (t, p, l) in zip(true_labels, predictions, label_mask)]

    # precisions = [precision_recall_curve(np.array(t[l]), np.array(p[l]))[0]
    #                 for (t, p, l) in zip(true_labels, predictions, label_mask)]

    # recalls = [precision_recall_curve(np.array(t[l]), np.array(p[l]))[1]
    #             for (t, p, l) in zip(true_labels, predictions, label_mask)]
...
def process_paths(apo_IDs, use_tensors=True, use_lm=False):
    ...
    for i, prot_bb in enumerate(pdbs):
        ...
        if use_lm:
            ...
            S_path = os.path.join(PRECOMPUTED_DIR, f'precompute_S/{fn}')
...
def predict_on_xtals(model, xtal_set, use_tensors=True, use_lm=False):
...
# Defaults preserve old behavior of the attention-enabled fork.
if 'use_attention' in training_config:
    use_attention = training_config['use_attention']
else:
    use_attention = True

if 'save_attention_weights' in training_config:
    save_attention_weights = training_config['save_attention_weights']
else:
    save_attention_weights = False

# custom filename/path for the saved attention weights (only used when
# save_attention_weights=True); defaults to 'attention_weights.npy' to match
# original behavior if not specified in the yaml
attention_weights_filename = training_config.get('attention_weights_filename', 'attention_weights.npy')

# -----END GVP INPUT PARAMETERS ---- #

# ---- XTAL VALIDATION AND TEST SETS -----#
# file contains pdb as well as labels
...
# use_tensors_in_test_set = training_config['use_tensors_in_test_set']
# ---- END XTAL VALIDATION AND TEST SETS -----#

# result dir
base_path = training_config['base_path']

# use tensors
# if True, need have use_tensors_in_test_set=False
# since a few pdbs doesn't have precomputed tensor files
# will cause errors
