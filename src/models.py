...
# Set to True to re-enable the verbose shape/debug prints that were present
# in the attention-development version of this file. Off by default since
# they print on every forward pass and are very noisy during training.
DEBUG = False
...
class MQAModel(Model):
    def __init__(self, node_features, edge_features,
        ...
        # filename (or path) used when save_attention_weights=True
        ...
        self.attention_weights_filename = attention_weights_filename
        ...
        # --- ATTENTION ---
        # Only created when use_attention=True, so that use_attention=False
        # is architecturally identical to the original PocketMiner model.
        if self.use_attention:
            self.snorm = LayerNormalization()
            self.scalar_attention = MultiHeadAttention(
                num_heads=attention_heads, key_dim=self.hs)
    ...
    def call(self, X, S, mask, train=False, res_level=False, ablate_aa_type=False):
        ...
        if DEBUG:
            tf.print("Shapes:", tf.shape(X), tf.shape(S), tf.shape(mask))
        ...
        # --- ATTENTION ---
        # Self-attention over the per-residue scalar features. Skipped
        # entirely when use_attention=False (plain PocketMiner behavior).
        if self.use_attention:
            if DEBUG:
                tf.print("h_V_out.shape:", tf.shape(h_V_out))

            norm_h_V_out = self.snorm(h_V_out)
            attended_scalars, attention_weights = self.scalar_attention(
                query=norm_h_V_out, key=norm_h_V_out, value=norm_h_V_out,
                return_attention_scores=True)

            if DEBUG:
                tf.print("attended_scalars.shape:", tf.shape(attended_scalars))
                tf.print("attention_weights.shape:", tf.shape(attention_weights))

            if train == False and self.save_attention_weights:
                # only save weights when making predictions, not during training
                np.save(self.attention_weights_filename, attention_weights)

            # combine original scalar features with the attended ones
            h_V_out = attended_scalars + h_V_out
...
class StructuralFeatures(Model):
    ...
    def call(self, X, mask):
        ...
        if not self.ablate_sidechain_vectors:
            if DEBUG:
                print(f'ablate_sidechain_vectors: {self.ablate_sidechain_vectors}')
